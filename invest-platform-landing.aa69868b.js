...
menu.onclick = (e)=>{
    e.preventDefault();
    if (menu.classList.contains('top-bar__btn-menu--close')) closeMenu();
    else openMenu();
};
aside.onclick = (e)=>{
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if (currentHash === '#menu') openMenu();
    [
        ...document.querySelectorAll('.lang__list li')
    ].forEach((li)=>{
        li.classList.remove('active');
        if (lang.textContent === li.textContent) li.classList.add('active');
    });
});
function openMenu() {
...
//# sourceMappingURL=invest-platform-landing.aa69868b.js.map
